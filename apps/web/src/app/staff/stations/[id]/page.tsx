          ...
          <>
            ...
            <section className="space-y-4"><div><h2 className="text-2xl font-semibold tracking-[-.03em]">Konnektörler</h2><p className="mt-1 text-sm text-muted-foreground">Aktiflik ve bakım durumunu buradan yönetin.</p></div>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{stationQuery.data.connectors.map((connector) => {
                ...
                return <Card key={connector.id} className="rounded-2xl border-white/80"><CardHeader><div className="flex items-start justify-between"><span className="flex size-10 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-700"><Zap className="size-5" /></span><Badge variant="outline" className={inMaintenance ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700'}>{inMaintenance ? 'Bakımda' : 'Aktif'}</Badge></div><CardTitle className="pt-2">{connector.code}</CardTitle><CardDescription>{connector.type === 'TYPE_2' ? 'Type 2' : 'CCS2'} · {connector.powerKw} kW · {connector.pricePerKWh} ₺/kWh</CardDescription></CardHeader><CardContent><Button variant="outline" className="w-full" disabled={statusMutation.isPending || (!inMaintenance && connector.currentStatus !== 'AVAILABLE')} onClick={() => statusMutation.mutate({ id: connector.id, status: inMaintenance ? 'ACTIVE' : 'MAINTENANCE' })}><Power className="size-4" />{inMaintenance ? 'Aktifleştir' : 'Bakıma al'}</Button></CardContent></Card>;
              })}</div>
            </section>
          </>
        ) : null}
